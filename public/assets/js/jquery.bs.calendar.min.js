            ...
            let weekContainer=$("<div>",{class:"d-flex flex-nowrap"}).appendTo(wrap);$("<div>",{class:"d-flex d-none justify-content-center align-items-center js-cal-row bootstrap-calendar-week",css:{fontSize:getFontSize(containerElement)+"px",color:"#adadad",width:widthHeight,height:widthHeight},html:['<small class="text-center mb-0 '+highlightClass+'">'+w+"</small>"].join("")}).appendTo(weekContainer);week.days.forEach(day=>{const isToday=today.formatDate(false)===day.formatDate(false);if(isToday)foundToday=true;const isInMonth=selectedDate.formatDate(true)[1]===day.formatDate(true)[1];const cellBackground=isToday?"  ":isInMonth?" fw-bold ":" fw-small ";const cellTextColor=isInMonth?"":"gray";const highlight=!isToday?"":" js-today ";let col=$("<div>",{"data-date":day.formatDate(false),class:"position-relative d-flex justify-content-center align-items-center bootstrap-calendar-day "+highlight+cellBackground,css:{color:cellTextColor,cursor:"pointer",height:widthHeight,width:"14.24%"},html:['<div style="font-size:'+"16"+'px">'+day.formatDate(true)[2]+"</div>",['<small class="js-count-events position-absolute text-center rounded-circle" style="width:4px; height:4px; bottom:4px; left: 50%; margin-left:-2px">',"</small>"].join("")].join("")}).appendTo(weekContainer);
                ...
                col.data("events",[])})});highlightDayName(containerElement);getEvents(containerElement,selectedDate,function(events){const setup=container.data("settings");$(containerElement).find("[data-date]").each(function(i,e){$(e).data("events",[])});events.forEach(event=>{const start=new Date(event.start);const end=new Date(event.end);let curDate=start.clone();let currDaysFormatted;do{currDaysFormatted=curDate.formatDate(false);let column=$(containerElement).find('[data-date="'+curDate.formatDate(false)+'"]');let dataEvents=[];if(column.length){dataEvents=column.data("events");dataEvents.push(event);column.data("events",dataEvents).find(`.js-count-events`).addClass("bg-danger")}if(column.length&&dataEvents.length&&setup.showPopover){$(column).popover("dispose");const popoverContent=setup.formatPopoverContent(dataEvents);const popoverSetup=$.extend(setup.popoverConfig||{},{title:"<small>"+getFormattedDate(curDate)+"</small>",content:popoverContent});column.popover(popoverSetup)}curDate=curDate.clone().addDays(1)}while(currDaysFormatted<end.formatDate(false))});if(widthHeight>=55){$(containerElement).find("[data-date]").each(function(i,e){const count=$(e).data("events").length;if(count){$(e).find(".js-count-events").css({fontSize:"9px",height:"16px",width:"16px",lineHeight:"16px",color:"white",left:"calc(50% - 8px)",bottom:"1px",margin:0}).text(count);console.log(count)}})}if(foundToday){$(containerElement).find(".js-today").trigger("click")}})}function events(containerElement){const settings=$(containerElement).data("settings");containerElement.on("click",".js-event",function(e){let $column=$(e.currentTarget);let event=$column.data("event");let container2=$column.closest(CONTAINER_WRAPPER_CLASS);container2.trigger("click-event",[event])}).on("click",".btn-prev-month",function(e){e.preventDefault();const btn=$(e.currentTarget);let container2=btn.closest(CONTAINER_WRAPPER_CLASS);const dateBefore=container2.data("current");const dateAfter=dateBefore.clone().subMonths(1);container2.data("current",dateAfter);drawCalendar(container2,dateAfter);container2.find(".js-collapse:not(.d-none)").addClass("d-none");container2.triggerAll("click-prev-month change-month")}).on("click",".btn-curr-month",function(e){e.preventDefault();
                    ...
                    container2.triggerAll("click-current-month change-month")}).on("click",".btn-next-month",function(e){e.preventDefault();
                        ...
                        container2.triggerAll("click-next-month change-month")}).on("mouseleave","[data-date]",function(e){if(settings.showPopover){$(e.currentTarget).popover("hide")}}).on("click","[data-date]",
    function(e){let $column=$(e.currentTarget);
                            let container2=$column.closest(CONTAINER_WRAPPER_CLASS);
                            container2.find("[data-date].active").removeClass("active");
                            $column.addClass("active");let date=new Date($column.data("date"));
                            let events=$column.data("events");container2.find(".js-day-name").html(date.showDateFormatted());drawEventList(container2,events,date);
                            container2.find(".js-collapse.d-none").removeClass("d-none");container2.trigger("change-day",[date,events])})}
    function drawEventList(containerElement,events,date){$(containerElement).trigger("show-event-list",[events]);
                const setup=containerElement.data("settings");
                let eventList=$(containerElement).find(".js-events");
                eventList.empty();if(!events.length){$("<div>",{class:"list-group-item",html:setup.formatNoEvent(date)}).appendTo(eventList)}else{events.forEach(event=>{let eventHtml=setup.formatEvent(event);
                    let eventWrapper=$("<div>",{class:"js-event col-4 d-flex justify-content-between align-items-center ",html:`<div class="flex-fill text-center">${eventHtml}</div><div><div class="btn-group-vertical btn-group-sm" role="group"></div></div>`}).appendTo(eventList);
                    getEventButtons(containerElement,event,eventWrapper.find(".btn-group-vertical"));
                    eventWrapper.data("event",event)})}$(containerElement).trigger("shown-event-list",[events])}function getEventButtons(containerElement,event,wrap){let settings=$(containerElement).data("settings");
